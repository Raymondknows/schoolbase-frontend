...
export default function AdminPeriodsManager({
  ...
  return (
    <div className="fixed inset-0 z-[70] flex items-center justify-center bg-black/50 px-4 py-6" role="dialog" aria-modal="true" aria-labelledby="periods-modal-title">
      <style>{`@keyframes timetable_periods_modal_enter { from { transform: translateX(36px) scale(.98); opacity: 0 } to { transform: translateX(0) scale(1); opacity: 1 } }`}</style>
      <section className="max-h-[min(760px,calc(100vh-48px))] w-full max-w-2xl overflow-y-auto rounded-md border border-border bg-surface shadow-[0_16px_50px_rgba(10,102,194,0.16)]" style={{ animation: "timetable_periods_modal_enter 320ms cubic-bezier(.2,.9,.2,1)" }}>
        <header className="flex items-start justify-between gap-4 border-b border-border/70 bg-brand/10 px-4 py-4 sm:px-6 sm:py-5">
          <div className="flex items-start gap-3"><div className="flex h-11 w-11 shrink-0 items-center justify-center border border-brand/20 bg-brand/10 text-brand"><Clock3 className="h-5 w-5" /></div><div><h2 id="periods-modal-title" className="text-lg font-semibold text-foreground">School periods</h2><p className="mt-1 text-sm text-muted">Adjust the default times to match your school day.</p></div></div>
          <button type="button" onClick={() => { playCloseTone(); onClose(); }} className="inline-flex h-8 w-8 items-center justify-center rounded-md border border-border text-muted hover:bg-background hover:text-foreground" aria-label="Close school periods" title="Close"><X className="h-4 w-4" /></button>
        </header>
        ...
        <div className="p-5 sm:p-6">
          <div className="flex items-end justify-between gap-4 border border-border bg-background p-4">
