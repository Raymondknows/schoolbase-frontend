...
import { ArrowRight, CheckCircle2, GitCompareArrows, X } from "lucide-react";
...
interface PublicComparisonShellProps {
  eyebrow: string;
  title: string;
  description: string;
  ...
  sectionTitle: string;
  ctaTitle: string;
  alternativeTone?: "neutral" | "warning";
}
    ...
    <main className="overflow-hidden bg-background">
            ...
            <div className="relative flex min-h-[360px] flex-col justify-between border border-brand/30 bg-white p-6 shadow-xl sm:min-h-[430px] sm:p-9">
              ...
              <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
                {proofTiles.slice(0, 6).map((item, index) => (
                  <div
                    key={item}
                    className={`${["bg-[#eaf4ff]", "bg-[#f1f7f4]", "bg-[#fff7e8]", "bg-[#f2efff]", "bg-[#edf7f8]", "bg-[#fff0f0]"][index % 6]} border border-black/5 p-4`}
                  >
              ...
              <div className="border-t border-border pt-5 text-sm text-muted">
                <span className="font-semibold text-brand">Fit matters.</span>{" "}
                Compare the workflows your school depends on.
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="mx-auto max-w-6xl px-6">
          <div className="max-w-3xl">
            ...
            <p className="mt-5 text-lg leading-8 text-muted">
              Look at the practical difference across the workflows that affect
            ...
            <table className="min-w-[700px] w-full border-collapse text-left">
              <thead>
                <tr className="border-b border-border bg-[#f6faff]">
                  ...
                  <th
                    className={`px-5 py-4 text-sm font-semibold ${alternativeTone === "warning" ? "text-amber-700" : "text-muted"}`}
                  >
                    {alternativeName}
                  </th>
              ...
              <tbody>
                {rows.map((row, index) => (
                  ...
                  >
                    ...
                    <td
                      className={`${index % 2 === 0 ? "bg-[#f6faff]" : "bg-[#edf5ff]"} px-5 py-5 text-sm font-semibold leading-6 text-brand`}
                    >
                      {row.schoolbase}
                    </td>
                  </tr>
      ...
      <section className="border-y border-border bg-[#f6faff] py-20 sm:py-24">
        <div className="mx-auto max-w-6xl px-6">
          <div className="max-w-3xl">
            <p className="text-sm font-semibold uppercase tracking-[0.2em] text-brand">
              What changes with SchoolBase
            </p>
            <h2 className="mt-4 text-3xl font-bold tracking-tight text-foreground sm:text-5xl">
              A clearer operating model for your school.
            </h2>
          </div>
          <div className="mt-12 grid gap-4 md:grid-cols-2 lg:grid-cols-3">
            {points.map(
              ({ icon: Icon, title, description: pointDescription }) => (
                <div
                  key={title}
