  ...
  return (
    <header className="relative overflow-hidden border border-border bg-surface px-6 pb-7 pt-10 sm:px-8 sm:pb-8 sm:pt-12">
      ...
      <div className="relative flex flex-col justify-between gap-6 lg:flex-row lg:items-end">
        <div>
          <div className="flex items-center gap-2 text-xs font-bold uppercase tracking-[.16em] text-brand"><Icon className="h-4 w-4" /> {eyebrow}</div>
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          {children}
          <Link href={actionHref} className="inline-flex items-center gap-2 rounded-md bg-brand px-4 py-2.5 text-sm font-semibold text-white shadow-sm transition hover:bg-brand-hover">{actionLabel}{actionLabel === "Dashboard" ? <ChevronRight className="h-4 w-4" /> : <ArrowUpRight className="h-4 w-4" />}</Link>
        </div>
      </div>
    </header>
  );
