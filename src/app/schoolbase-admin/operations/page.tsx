...
export default function OperationsPage() {
  ...
  return (
    <main className="operations-page min-h-screen bg-background px-2 py-4 text-foreground sm:px-8 sm:py-6 lg:px-12">
      <style>{`@keyframes operations-pulse { 0%,100% { opacity: 1; box-shadow: 0 0 0 0 rgb(10 102 194 / .4) } 50% { opacity: .55; box-shadow: 0 0 0 7px rgb(10 102 194 / 0) } } @keyframes operations-scan { from { transform: translateX(-100%) } to { transform: translateX(100%) } } @keyframes operations-bar-sweep { from { transform: translateX(-120%) } to { transform: translateX(300%) } } .operations-pulse { animation: operations-pulse 1.8s ease-in-out infinite } .operations-grid { background-image: linear-gradient(to right, rgb(148 163 184 / .10) 1px, transparent 1px), linear-gradient(to bottom, rgb(148 163 184 / .10) 1px, transparent 1px); background-size: 24px 24px; } .operations-scan { animation: operations-scan 5s linear infinite; } .operations-page > div > header, .operations-page > div > section, .operations-page .grid > div { border-radius: 0 !important; box-shadow: none !important; } .operations-status-bar { position: relative; display: block; width: 100%; height: .5rem; overflow: hidden; } .operations-status-bar::after { content: ''; position: absolute; inset: 0 auto 0 0; width: 28%; background: linear-gradient(90deg, transparent, rgb(255 255 255 / .85), transparent); animation: operations-bar-sweep 2.2s ease-in-out infinite; } @media (prefers-reduced-motion: reduce) { .operations-pulse, .operations-scan, .operations-status-bar::after { animation: none } }`}</style>
