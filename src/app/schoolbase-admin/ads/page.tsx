...
export default function PlatformAdsPage() {
  ...
  async function loadData() {
    try {
      ...
      setNotice(error instanceof Error ? error.message : "Unable to load ads data.");
    } finally {
      setLoading(false);
    }
  }
  ...
  async function submitForm(event: React.FormEvent, kind: "advertiser" | "campaign") {
    ...
    try {
      ...
      playCloseTone();
      setDialog(null);
      setNotice(`${kind === "advertiser" ? "Advertiser" : "Campaign"} created successfully.`);
  ...
  async function updateCampaign(id: string, action: "approve" | "reject" | "pause" | "submit" | "live") {
    ...
    try {
      ...
      setNotice(`Campaign ${action === "approve" ? "approved" : action === "reject" ? "rejected" : action === "submit" ? "submitted" : action === "live" ? "live" : "paused"}.`);
      await loadData();
    } catch (error) {
      setNotice(error instanceof Error ? error.message : "Unable to update campaign.");
    } finally {
      setSaving(false);
    }
  }
  ...
  async function updateAdvertiser(id: string, action: "verify" | "reject") {
    ...
    try {
      ...
      setNotice(`Advertiser ${action === "verify" ? "verified" : "rejected"}.`);
      await loadData();
    } catch (error) {
      setNotice(error instanceof Error ? error.message : "Unable to update advertiser.");
    } finally {
      setSaving(false);
    }
  }
  ...
  return (
    <main className="min-h-screen bg-background px-2 py-4 text-foreground sm:px-8 sm:py-6 lg:px-12">
      <div className="mx-auto max-w-7xl space-y-6">
        ...
        <div className="border border-border bg-surface">
          ...
          {loading ? (
            ...
            <div className="space-y-6 p-6">
              ...
              {activeTab === "Approvals" && <div className="border border-border bg-background p-4 text-sm text-muted">Use the review queue below to submit, approve, or reject campaigns.</div>}
        ...
        {notice && <div className="border border-brand/30 bg-brand/5 px-4 py-3 text-sm text-foreground">{notice}</div>}
      </div>

      {activeTab === "Approvals" && !loading && (
        <div className="mx-auto mt-6 max-w-7xl border border-border bg-surface p-6">
          <div className="mb-4 flex items-center justify-between gap-3">
            <div><h2 className="text-lg font-semibold text-foreground">Manual review queue</h2><p className="mt-1 text-sm text-muted">Submit drafts, then review submitted campaigns before public delivery.</p></div>
