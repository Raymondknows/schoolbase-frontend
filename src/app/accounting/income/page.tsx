...
export default function IncomePage() {
  ...
  async function loadData() {
    try {
      ...
      const [categoriesRes, transactionsRes] = await Promise.all([
        ...
        fetch('/api/bursar/cashbook?type=INCOME', { credentials: 'include' }),
      ]);
      ...
      setCategories(categoriesData.filter((c: Category) => c.type === 'INCOME'));
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const response = await fetch('/api/bursar/income', {
        method: 'POST',
      ...
      if (!response.ok) {
        ...
        throw new Error(data.error || 'Failed to create income entry');
      }
              ...
              <tbody>
                {transactions.map((transaction) => (
                  <tr key={transaction.id} className="border-t border-border hover:bg-background/50 transition-colors">
                    ...
                    <td className="px-4 py-3 font-semibold text-green-600">{formatAmount(transaction.amount)}</td>
