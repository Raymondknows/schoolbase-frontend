...
export default function AboutPage() {
  return (
    <div className="overflow-hidden bg-background">
      ...
      <section className="border-y border-border bg-[#f6faff] py-20 sm:py-24"><div className="mx-auto max-w-6xl px-6"><div className="max-w-3xl"><p className="text-sm font-semibold uppercase tracking-[0.2em] text-brand">Why schools choose us</p><h2 className="mt-4 text-3xl font-bold tracking-tight text-foreground sm:text-5xl">A complete foundation without unnecessary complexity.</h2></div><div className="mt-12 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">{['Go live in as little as 48 hours', 'Fees, results, communication, and administration in one platform', 'Transparent plans with no hidden costs', 'Dedicated support for your school team'].map((reason) => <div key={reason} className="flex items-start gap-3 border border-border bg-white p-5"><CheckCircle2 className="mt-0.5 h-5 w-5 shrink-0 text-brand" /><span className="text-sm leading-7 text-foreground">{reason}</span></div>)}</div></div></section>
