...
export default function ClassPage() {
  ...
  return (
    <main className="min-h-screen pb-12">
      <div className="mx-auto max-w-7xl space-y-6 px-2 py-8 sm:px-8 lg:px-12">
      <TeacherPageHeader icon={BookOpen} title={selectedClass?.name || 'My Class'} description="View your students, monitor roster health, and manage your class workspace." count={selectedClass ? `${students.length} students` : 'No class selected'}>
        {classes.length > 1 && <label htmlFor="class-selector" className="flex items-center gap-2 text-xs font-semibold text-muted"><span className="sr-only">Switch class</span><select id="class-selector" value={selectedClass?.id || ''} onChange={(event) => { const cls = classes.find((item) => item.id === event.target.value); setSelectedClass(cls || null); setSearchQuery(''); setCurrentPage(1); }} className="max-w-[180px] rounded-md border border-border bg-background px-3 py-2.5 text-sm font-semibold text-foreground outline-none transition focus:border-brand">{classes.map((cls) => <option key={cls.id} value={cls.id}>{cls.name}{cls.arm ? ` - ${cls.arm}` : ''}{cls.phase ? ` - ${cls.phase}` : ''}</option>)}</select></label>}
      </TeacherPageHeader>
